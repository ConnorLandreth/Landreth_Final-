...
```{r}
# One Numerical - Years of Experience
ggplot(SD, aes(x = Years.of.Experience, fill = ..x..)) +
  ...
  theme(
    ...
    plot.caption = element_text(face = "bold", size = 10, hjust = 0, margin = margin(t = 20)),
    axis.title.x = element_text(size = 14),
    axis.title.y = element_text(size = 14),
    axis.text = element_text(size = 12),
    panel.grid.major = element_blank(),
...
# One Categorical - Education Level
ggplot(SD %>% filter(!is.na(Education.Level)), aes(x = Education.Level)) +
  geom_bar(fill = "skyblue", color = "steelblue") +
  geom_text(stat = 'count', aes(label = ..count..), vjust = -0.5, size = 3, color = "black") +  # Add count labels above the bars
  labs(title = "Distribution of Education Levels", subtitle = "United States, 2022", x = "Education Level", y = "") +  # Remove y-axis label
  theme_minimal() +
